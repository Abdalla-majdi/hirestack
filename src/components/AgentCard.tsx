...
export const AgentCard = ({
  ...
  return <Card className="group h-full flex flex-col transition-all duration-300 hover:shadow-xl border-border/50 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-accent/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      
      <CardHeader className="relative pb-4">
        <div className="flex items-start gap-4">
          <div className="relative">
            <img src={avatar} alt={name} className="w-20 h-20 rounded-full object-cover ring-4 ring-background shadow-lg" />
      ...
      <CardFooter className="relative flex-col gap-3">
        <Button onClick={onHire} className="w-full transition-all duration-300 hover:scale-105" size="lg">
          Hire {name}
        </Button>
